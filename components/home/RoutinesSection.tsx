...
import { Star, Clock, Users, ShoppingBag, Sparkles, Flame } from "lucide-react";
...
export default function RoutinesSection() {
  ...
  const [activeType, setActiveType] = useState<string>(() => {
    const t = getRoutineTypes();
    return t.some((x) => x.type === "daily") ? "all" : t[0]?.type || "daily";
  });
  ...
  // slower auto-advance: one slide per tick
  useEffect(() => {
  ...
  return (
    <section id="routines" className="w-full scroll-mt-28 bg-gradient-to-b from-white via-primary/5 to-white py-8 sm:py-10 lg:py-12">
      <Container>
        <SectionTitle
          eyebrow={isAr ? "روتينات مخصصة" : "Personalized Routines"}
          title={isAr ? "روتينات يومية متكاملة" : "Integrated Daily Routines"}
          subtitle={
            isAr
              ? "اختاري روتينك بثلاثة مستويات (أساسي، قياسي، متقدم) وكل خطوة مدروسة لتحقيق أفضل نتيجة"
              : "Pick your routine in three levels (Essential, Standard, Premium) — every step curated for the best results"
          }
          action={
            <Link
              href="/products"
        ...
        <div className="relative" dir="ltr">
          ...
          <div className="overflow-hidden py-6" role="region" aria-label={isAr ? 'روتينات متحركة' : 'Routines showcase'}>
            <div
              ...
              style={{
                transform: `translate3d(${-slideIndex * slideW}px, 0, 0)`,
            ...
            >
              {looped.map((routine, idx) => (
                ...
                >
                  <RoutineCard routine={routine} index={idx % filtered.length} isAr={isAr} />
                </div>
              ))}
            </div>
          </div>
...
function LevelTab({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-pill border px-4 py-1.5 text-xs font-bold transition-all duration-200 ease-out-smooth ${
        active
      ...
      <div className="flex flex-1 flex-col p-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-1 rounded-pill bg-primary/10 px-2.5 py-1 text-[11px] font-bold text-primary">
        ...
        {/* Meta — rating, buyers, duration */}
        <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-border">
          <span className="inline-flex items-center gap-1.5 text-sm text-muted">
            <Star size={13} className="fill-amber-400 text-amber-400 shrink-0" />
            <span className="font-medium text-gray-600">{safeRatingDisplay(routine)}</span>
            <span className="text-gray-400">({safeReviewCountDisplay(routine)})</span>
          </span>
          <span className="inline-flex items-center gap-1.5 text-sm text-muted">
            <Users size={13} className="shrink-0" />
            <span className="font-medium text-gray-600">{safeBuyersDisplay(routine).toLocaleString("ar-YE")}</span>
            <span className="text-gray-400">{isAr ? "عميلة" : "buyers"}</span>
          </span>
          <span className="inline-flex items-center gap-1.5 text-sm text-muted">
            <Clock size={13} className="shrink-0" />
            <span className="font-medium text-gray-600">{routine.duration}</span>
          </span>
        </div>

        {/* Price + CTA */}
        <div className="mt-auto flex items-center justify-between border-t border-border pt-4 space-x-4">
          <div className="space-y-1">
            <p className="text-lg font-extrabold text-foreground">
              {formatPrice(discountedPrice)}
              <span className="ms-1 text-[10px] font-normal text-muted">{isAr ? "ر.ي" : "YER"}</span>
            </p>

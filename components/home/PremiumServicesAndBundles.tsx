...
/* MAIN SECTION                           */
/* ================================================================================================ */
export default function PremiumServicesAndBundles() {
  const { lang } = useLang()
  const isAr = lang === "ar"
  const t = (ar: string, en: string) => (isAr ? ar : en)

  const topBundles = useMemo(() => listBundles().slice(0, 4), [])
    ...
    <section id="bundles" className="w-full scroll-mt-28 bg-white py-8 sm:py-10 lg:py-12">
      <Container>
        ...
        <div className="mb-12">
          ...
          <SectionTitle
            eyebrow={<>
              ...
              {t("باقات لكل مناسبة", "Bundles for Every Occasion")}
            </>}
            title={t("هدايا وباقات مختارة بعناية", "Carefully Selected Bundles")}
            subtitle={t("باقات جاهزة لكل مناسبة — اختاري الباقة المثالية أو صممي باقتك الخاصة", "Ready bundles for every occasion — pick the perfect one or design your own")}
            action={
              <Link
                href="/bundles"
        ...
        <SectionTitle
          eyebrow={<>
            <span className="h-0.5 w-6 rounded-pill bg-accent" />
            {t("احترافية ورعاية", "Professional Care")}
          </>}
          title={t("خدمات تجميلية", "Beauty Services")}
          subtitle={t("فريق متخصص وتقنيات حديثة لعناية تعكس أناقتك", "A specialized team and modern techniques for care that reflects your elegance")}
          action={
            <Link
              href="/contact"
        ...
        <div className="relative mx-auto mt-8 w-full max-w-5xl scroll-mt-28 overflow-hidden rounded-[2.5rem] border border-primary/15 bg-gradient-to-br from-[#FFF7F2] via-primary-50 to-secondary-50 p-6 sm:p-8">
          ...
          <div className="relative grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {SERVICES.map((service, i) => (
              ...
              <Link
                href={service.href}
                className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition-all duration-500 hover:-translate-y-1.5 hover:border-primary/25 hover:shadow-[0_18px_40px_-16px_rgba(75,42,111,0.28)]"
              >
                ...
                <div className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-full border border-accent/25 bg-accent-50 text-accent transition-all duration-500 group-hover:scale-110 group-hover:bg-gradient-to-br group-hover:from-primary group-hover:to-secondary group-hover:text-accent-light group-hover:shadow-md group-hover:shadow-primary/30">
                ...
                {/* Availability — "قريباً" */}
                {!service.available ? (
                  <div className="mt-4 inline-flex items-center gap-2 self-start rounded-full bg-gradient-to-l from-primary to-secondary px-4 py-2 text-xs font-bold text-white shadow-[0_8px_20px_-8px_rgba(75,42,111,0.5)] sm:text-sm">
                    <Clock size={14} className="shrink-0 text-accent-light" />
                    {t("قريباً", "Coming Soon")}
                  </div>
                ) : (
                  <div className="mt-4 inline-flex items-center gap-1.5 self-start text-[11px] font-bold text-primary transition-all duration-300 group-hover:gap-2.5">

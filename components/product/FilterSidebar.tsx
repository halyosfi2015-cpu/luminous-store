...
const brandOptions = ["CeraVe", "COSRX", "La Roche-Posay", "The Ordinary", "Neutrogena", "SkinCeuticals"];
...
const skinTypeEn: Record<SkinType, string> = {
  dry: "Dry", oily: "Oily", combination: "Combination", sensitive: "Sensitive", normal: "Normal", all: "All Types",
};

const skinConcernEn: Record<SkinConcern, string> = {
  acne: "Acne", dryness: "Dryness", pigmentation: "Pigmentation", aging: "Aging",
  redness: "Redness", large_pores: "Large Pores", uneven_texture: "Uneven Texture",
  dark_circles: "Dark Circles", oiliness: "Oiliness", sensitivity: "Sensitivity",
};
...
function Checkbox({
  ...
  return (
    <label onClick={onChange} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-muted-bg group">
      <div className={`flex h-4 w-4 items-center justify-center rounded border-2 transition-all duration-150 ${
      ...
      <FilterGroup label="نوع البشرة">
        {skinTypeOptions.map((type) => (
          <Checkbox
            ...
            label={`${skinTypeLabels[type]} (${skinTypeEn[type]})`}
      ...
      <FilterGroup label="مشاكل البشرة">
        {skinConcernOptions.map((concern) => (
          <Checkbox
            ...
            label={`${skinConcernLabels[concern]} (${skinConcernEn[concern]})`}
      ...
      <FilterGroup label="التقييم">
        {ratingOptions.map((stars) => (
          <label
            ...
            className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-muted-bg group"
          >
            <div className={`flex h-4 w-4 items-center justify-center rounded border-2 transition-all duration-150 ${
              filters.ratings.includes(stars) ? "bg-primary border-primary" : "border-border-strong group-hover:border-primary/50"
            }`}>
              {filters.ratings.includes(stars) && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>}
            </div>
            <div className="flex items-center gap-0.5">
              {Array.from({ length: stars }, (_, i) => (
                <Star key={i} size={13} className="fill-accent text-accent" />
              ))}
              <span className="me-1 text-sm text-muted">+</span>
            </div>
          </label>
        ))}
      </FilterGroup>

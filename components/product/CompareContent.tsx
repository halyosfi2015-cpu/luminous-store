        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            ...
            <CompareRow
              label="الوصف"
              cells={compared.map((p) => (
                <span key={p.id} className="text-xs leading-relaxed">{productCompareDetails[p.id]?.descriptionAr ?? "—"}</span>
              ))}
            />
            <CompareRow
            ...
            <CompareRow
              ...
              cells={compared.map((p) => (
                <ul key={p.id} className="space-y-1 text-xs">
                  {(productCompareDetails[p.id]?.ingredientsAr ?? []).slice(0, 4).map((ingName, i) => (
                    <li key={i} className="text-muted">
                      {ingName}
                    </li>
                  ))}
                  {(productCompareDetails[p.id]?.ingredientsAr ?? []).length > 4 && (
                    <li className="text-[11px] text-primary">+{(productCompareDetails[p.id]?.ingredientsAr ?? []).length - 4} أخرى</li>
                  )}
                </ul>
              ))}
            />

  ...
  const toggle = useCallback((productId: string) => {
    setItems((prev) => {
      if (prev.includes(productId)) {
        trackClient({
          event_type: ANALYTICS_EVENT_TYPES.PRODUCT_REMOVED_FROM_WISHLIST,
          entity_type: "product",
          entity_id: productId,
          properties: { action: "remove_from_compare" },
        });
        return prev.filter((id) => id !== productId);
      }
      if (prev.length >= MAX_COMPARE) return prev;
      trackClient({
        event_type: ANALYTICS_EVENT_TYPES.PRODUCT_ADDED_TO_WISHLIST,
        entity_type: "product",
        entity_id: productId,
        properties: { action: "add_to_compare" },
      });

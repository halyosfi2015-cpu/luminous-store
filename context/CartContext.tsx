...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addItem = useCallback((item: CartItem) => {
    trackClient({
      event_type: ANALYTICS_EVENT_TYPES.PRODUCT_ADDED_TO_CART,
      entity_type: "product",
      entity_id: item.productId,
      properties: { quantity: item.quantity, price: item.price },
    });
    dispatch({ type: "ADD_ITEM", payload: item });
  }, []);
  const removeItem = useCallback((productId: string) => {
    trackClient({
      event_type: ANALYTICS_EVENT_TYPES.PRODUCT_REMOVED_FROM_CART,
      entity_type: "product",
      entity_id: productId,
    });
  ...
  const updateQuantity = useCallback((productId: string, quantity: number) => {
    trackClient({
      event_type: ANALYTICS_EVENT_TYPES.CART_UPDATED,
      entity_type: "product",
      entity_id: productId,
      properties: { quantity },
    });
  ...
  const toggleCart = useCallback(() => {
    trackClient({ event_type: ANALYTICS_EVENT_TYPES.CART_VIEW });
